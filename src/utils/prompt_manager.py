...
class PromptManager:
    ...
    def render(self, template_name: str, **kwargs) -> str:
        try:
            ...
            raise ValueError(
                f"Error rendering template '{template_name}': {str(e)} (Search path: {self.templates_dir})"
            )

    def render_project_blueprint(
        self,
        user_prompt: Optional[str] = None,
        system_info: Optional[Dict[str, Any]] = None,
        problem_statement_language: str = "others",
    ) -> str:
        lang = problem_statement_language or "others"
        if lang.lower() == "cuda":
            return self.render(
                "project_blueprint_cuda.j2",
                user_prompt=user_prompt,
                system_info=system_info,
            )
        return self.render(
            "project_blueprint.j2", user_prompt=user_prompt, system_info=system_info
        )
    ...
    def render_file_descriptor(self, software_blueprint, folder_structure, file_name):
        return self.render(
            "file_descriptor.j2",
