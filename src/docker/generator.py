...
class DockerTestFileGenerator:
    def __init__(self, project_root: str, software_blueprint: Dict,
                 ...
                 provider=None, problem_statement_language: str = "others"):
    ...
    def generate_test_dockerfile_blueprint(self) -> List[Dict]:
        ...
        blueprint_file_name = "test_dockerfile_blueprint.j2"
        if(self.problem_statement_language.lower() == "cuda"):
            blueprint_file_name = "test_dockerfile_blueprint_cuda.j2"

        prompt = self.pm.render(blueprint_file_name,
    ...
    def generate_shell_test_script(self, test_files: List[str]) -> Optional[str]:
        ...
        messages = [{"role": "user", "content": prompt}]
        response = self.provider.call_model(messages)
        script_content = self.provider.extract_text(response)
        ...
        script_path = os.path.join(self.project_root, "run_tests.sh")
        ...
        self.error_tracker.log_change(
            ...
            error_context="Test shell script generation phase for CUDA project",
            actions=["generate_test_shell_script"]
        )
    ...
    def generate_all(self) -> Dict:
        ...
        try:
            ...
            if(self.problem_statement_language.lower() == "cuda"):
                shell_script_success = self.generate_shell_test_script(test_files)
                results["test_shell_script"] = shell_script_success
            else:
                dockerfile_success = self.generate_dockerfile()
                results["dockerfile"] = dockerfile_success
            ...
            results["success"] = (shell_script_success if self.problem_statement_language.lower() == "cuda" else dockerfile_success) and len(test_files) > 0
